   ...
   #define SNR_LIBVER      "Library Version: 24.03.02"
   ...
   void sBuy ( double safSLV=0 , double safTPV=0 , double safPercCalcSLV=0 , double safLot=-1 , string safComment="" , int safTriesMax=10 ) {
      //// goDebug ( __FUNCTION__ ) ;
      // -------------------- x
      ...
      if ( glb_BeaconMode == true ) {
         ...
         glb_LastTradeTime = StringSubstr ( goGetDateTime() , 0 , 10 ) ; glb_LastTradeType = "B" ;
         return ; }
      // -------------------- x
      ...
      while ( safTradeSuccess == false ) {
         ...
         Sleep ( 1000 ) ; safTriesCount += 1 ; if ( safTriesCount > safTriesMax ) { goPrint ( "Maximum number of tries reached" ) ; break ; }}
      if ( safTradeSuccess == true ) { glb_LastTradeTime = StringSubstr ( goGetDateTime() , 0 , 10 ) ; glb_LastTradeType = "B" ; }}

   void sSell ( double safSLV=0 , double safTPV=0 , double safPercCalcSLV=0 , double safLot=-1 , string safComment="" , int safTriesMax=10 ) {
      //// goDebug ( __FUNCTION__ ) ;
      // -------------------- x
      ...
      if ( glb_BeaconMode == true ) {
         goBroadcast_SIG ( goTele_PrepMsg ( "S" , "0" , "1" , string ( sPow ( safSLV ) ) , string ( sPow ( safTPV ) ) ,
            string ( sPow ( safPercCalcSLV ) ) , "0.5" , string ( ND2 ( safLot ) ) , "0" , string ( glb_LotSize ) , safComment ) ) ;
         glb_LastTradeTime = StringSubstr ( goGetDateTime() , 0 , 10 ) ; glb_LastTradeType = "S" ;
         return ; }
      // -------------------- x
      bool safTradeSuccess = false ;
      int safTriesCount = 0 ;
      ...
      while ( safTradeSuccess == false ) {
         ...
         Sleep ( 1000 ) ; safTriesCount += 1 ; if ( safTriesCount > safTriesMax ) { goPrint ( "Maximum number of tries reached" ) ; break ; }}
      if ( safTradeSuccess == true ) { glb_LastTradeTime = StringSubstr ( goGetDateTime() , 0 , 10 ) ; glb_LastTradeType = "S" ; }}
   ...
   void goKillAccount_Execute ( double StartBalance, int safPercent=90 ) {
      //// goDebug ( __FUNCTION__ ) ;
   ...
   void goOnTick () {
      ...
      goAdd2Array ( string ( safBalance ) , glb_BAL ) ;
      LastBalance = safBalance ;
      goAdd2Array ( string ( safEquity ) , glb_EQU ) ;
      LastEquity = safEquity ;
      goAdd2Array ( string ( TimeGMT() ) , glb_TIME ) ; }
   ...
   void goAdd2Array ( string safNewLine , string &safArray[] ) {
      //// goDebug ( __FUNCTION__ ) ;
   ...
   void goServer_GetFileList ( string safURL , string &safFNArray[] ) {
      ...
      for ( int x = 0 ; x < ArraySize ( result ) ; x++ ) {
         ...
         if ( StringLen ( result [ x ] ) > 0 ) {
            goAdd2Array ( result [ x ] , safFNArray ) ; }}}
   ...
   string goSignal_RSI_2_3 ( ENUM_TIMEFRAMES safTF , int safTrigger=55 ) {
      //// goDebug ( __FUNCTION__ ) ;
      ...
      glb_EAP = safTF ;
         ...
         double RSI_2 = B0 [ glb_FC ] ;
         if ( ind_RSI ( 3 ) == false ) { return "X" ; }
         double RSI_3 = B0 [ glb_FC ] ;
         // -------------------- Calc signal here
   ...
   bool goTester_OnInIt ( string safHeader , string safFN , string safAddon ) {
      //// goDebug ( __FUNCTION__ ) ;
      // -------------------- Check version
      if ( goSecurity_VersionCheck ( safFN ) == false ) { ExpertRemove () ; return ( false ) ; }
      ...
      for ( int x = 0 ; x < ArraySize ( safFileNameArray ) ; x++ ) {
         ...
         for ( int y = 0 ; y < ArraySize ( safFC ) -1 ; y++ ) {
            ...
            if ( StringFind ( UT ( safFC [ y ] ) , "PASS" , 0 ) < 0 ) {
               goAdd2Array ( ( safFileNameArray [ x ] + "," + safFC [ y ] ) , safAllFC ) ; }}}
   ...
   void goDashboard_ReportOnBots ( string safType , string safFN , string safSinceDate , string &safLastOPS[] ) {
      ...
      goSNR_HTML_Header ( "345" ) ;
         ...
         for ( int x = 0 ; x < ArraySize ( glb_SymbolArray ) ; x++ ) {
            ...
            for ( int y = 0 ; y < ArraySize ( safLastOPS ) ; y++ ) {
               if ( StringFind ( safLastOPS [ y ] , "|" + glb_SymbolArray [ x ] ) >= 0 ) {
                  goAdd2Array ( safLastOPS [ y ] , safSelectArray ) ; }}
         ...
         for ( int x = 0 ; x < ArraySize ( safBotsSplit ) ; x++ ) {
            ArrayResize ( safSelectArray , 0 ) ;
            for ( int y = 0 ; y < ArraySize ( safLastOPS ) ; y++ ) {
               goAdd2Array ( ( safLastOPS [ y ] + "|" + goFind_BotType ( safLastOPS [ y ] ) + "|" ) , safSelectArray ) ; }
