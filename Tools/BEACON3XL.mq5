   ...
   input string            myMagic                 = "TEST"       ; // EA Magic Number
   input string            myMagicSuffix         = ""           ; // EA Magic Suffix
////   input int               iMaxAllowedDD           = 20           ; // Quit test beyond this DD [0=Disabled]
////   int                     iMaxAllowedDays         = 0            ; // Quit test if no trades for X Days [0=Disabled]
   bool              iUseSL                  = false        ; // Use SL
   enumRiskLevel     RiskLevel               = High         ; // Risk appetite
   ...
   input string            mySunsetDate            = ""           ; // EA Sunset Date [empty=Disabled]
...
////         if ( goTester_CheckElapseTime () == false ) { ExpertRemove() ; return ; }}
      // -------------------- Expiry date check
      goLocalFile_Write ( "Beacon started" , myMagicSuffix ) ;
...
////      } else {
      ...
      goBroadcast_OPS ( goTele_PrepMsg ( "BEACON" , "STARTED" , safStatusPointer , SNR_LIBVER , SNR_BOTVER ) ) ; }
...
/*
   ...
   void OnTesterDeinit () {
      ...
      glb_ServerFileName = "TestLog.txt" ;
      // -------------------- Write file content to server
      string safMsg = safBrokerCurr + "," + safBotName + safSLAddon + "," + mySettings ;
      safMsg += (string) int ( TesterStatistics ( STAT_TRADES  ) ) + "," ;
      safMsg += (string) ND2 ( TesterStatistics ( STAT_PROFIT  ) ) + "," ;
      safMsg += (string) ND2 ( TesterStatistics ( STAT_EQUITYDD_PERCENT  ) ) + "," ;
      safMsg += (string) ND2 ( TesterStatistics ( STAT_RECOVERY_FACTOR  ) ) + "," ;
      safMsg += (string) ND2 ( TesterStatistics ( STAT_SHARPE_RATIO  ) ) + "," ;
      safMsg += (string) int ( TesterStatistics ( STAT_PROFIT ) / TesterStatistics ( STAT_EQUITYDD_PERCENT  ) ) ;
